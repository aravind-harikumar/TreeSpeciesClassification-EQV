function [AccTS, label_TS, teststats] = MainSVM_CV(TRs,TSs)%addpath('./functions')
...
elseif(length(unique(label_TS)) == 2)
    OA = (CM_TS(1,1) + CM_TS(2,2))*100/ size(tsSet,1)% sum(CM_TS(:));
elseif(length(unique(label_TS)) == 3)
    OA = (CM_TS(1,1) + CM_TS(2,2) + CM_TS(3,3))*100/ size(tsSet,1)% sum(CM_TS(:));
elseif(length(unique(label_TS)) == 5)
    OA = (CM_TS(1,1) + CM_TS(2,2) + CM_TS(3,3) + CM_TS(4,4) + CM_TS(5,5))*100/ size(tsSet,1)% sum(CM_TS(:));
elseif(length(unique(label_TS)) == 6)
    OA = (CM_TS(1,1) + CM_TS(2,2) + CM_TS(3,3) + CM_TS(4,4) + CM_TS(5,5) + CM_TS(6,6))*100/ size(tsSet,1)% sum(CM_TS(:));
else
    OA = (CM_TS(1,1) + CM_TS(2,2))*100/ size(tsSet,1)% sum(CM_TS(:));
end
