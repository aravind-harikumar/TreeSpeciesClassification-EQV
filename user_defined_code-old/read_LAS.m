...
try
data=[X Y Z R I C S]; % to fix read error in column 7 , pls change back
catch
minl = min([length(X) length(Y) length(Z) length(R) length(I) length(C) length(S)]);
data=[X(1:minl) Y(1:minl) Z(1:minl) R(1:minl) I(1:minl) C(1:minl) S(1:minl)]; % to fix read error in column 7 , pls change back
end
